...
1. 데이터셋에 날짜 컬럼이 존재하고
2. API 사용 시 그 날짜 컬럼으로 ORDER BY DESC 되어 가져온다는 가정하에 사용 가능
'''

class SeoulApiDateSensor(BaseSensorOperator): #SeoulApiDataSensor 클래스 선언 - BaseSensorOperator 클래스로부터 상속
    template_fields = ('endpoint',)

    def __init__(self, dataset_nm, base_dt_col, day_off=0, **kwargs):
        '''
        dataset_nm: 서울시 공공데이터 포털에서 센싱하고자 하는 데이터셋명
    ...
    def poke(self, contenxt):
        ...
        last_date = last_dt[:10].replace('.', '-').replace('/', '-')
