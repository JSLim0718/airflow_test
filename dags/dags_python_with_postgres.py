...
with DAG(
    ...
    schedule = None,
    start_date = pendulum.datetime(2025, 8, 31, tz='Asia/Seoul'),
    catchup = False
) as dag:
    ...
    def insert_postgres(ip, port, dbname, user, passwd, **kwargs):
        ...
        with closing(pyycopg2.connect(host=ip, dbname=dbname, user=user, password=passwd, port=int(port))) as conn:
